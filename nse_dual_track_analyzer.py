...
TEMP_FILE_PREFIX = ".tmp_stock_agent_"
...
def _atomic_write_text(path_obj, content, encoding="utf-8"):
    ...
    tmp_name = None
    try:
        with tempfile.NamedTemporaryFile("w", delete=False, encoding=encoding, dir=str(path_obj.parent), prefix=TEMP_FILE_PREFIX, suffix=path_obj.suffix or ".tmp") as tmp:
            tmp.write(content)
            tmp_name = tmp.name
...
def _atomic_write_csv(df, path_obj):
    ...
    tmp_name = None
    try:
        with tempfile.NamedTemporaryFile("w", delete=False, encoding="utf-8", newline="", dir=str(path_obj.parent), prefix=TEMP_FILE_PREFIX, suffix=path_obj.suffix or ".csv") as tmp:
...
def cleanup_stale_temp_files(base_dir, max_age_hours=24):
    """Delete old temporary files created by this script only.
    
    Matches:
      - Files with our scoped prefix (.tmp_stock_agent_*)
      - Old-style Python tempfile random names (tmp*.xlsx, tmp*.csv, tmp*.txt)
    Does NOT touch analysis_history/ archives.
    """
    ...
    removed = 0
    # Our prefixed temp files + old random-name temp files (not in subdirs)
    patterns = [f"{TEMP_FILE_PREFIX}*", "tmp*.xlsx", "tmp*.csv", "tmp*.txt"]
            ...
            for tbl in soup.find_all("table"):
                ...
                if _growth_section in ("sales", "profit"):
                    for row in rows[1:]:
                        ...
                        if _growth_section == "sales":
                            if "3" in key:
                                data["sales_growth_3y"] = val
                            elif "5" in key:
                                data["sales_growth_5y"] = val
                            elif "10" in key:
                                data["sales_growth_10y"] = val
                            elif "7" in key and "sales_growth_10y" not in data:
                                data["sales_growth_7y"] = val
                        elif _growth_section == "profit":
                            if "3" in key:
                                data["profit_growth_3y"] = val
                            elif "5" in key:
                                data["profit_growth_5y"] = val
                            elif "10" in key:
                                data["profit_growth_10y"] = val
                            elif "7" in key and "profit_growth_10y" not in data:
                                data["profit_growth_7y"] = val
                    _growth_section = None
                    continue
...
def format_growth_pct(v):
    """Format growth values as percentage strings for Excel export."""
    num = parse_float(v)
    if num is None:
        return "Data unavailable"
...
# ===== OWNERSHIP FALLBACK FIX =====
    ...
    return {
        ...
        "Rev. Growth 5Y (%)": format_growth_pct(f.get("sales_growth_5y")),
        "Rev. Growth 10Y/7Y (%)": format_growth_pct(f.get("sales_growth_10y") or f.get("sales_growth_7y")),
        "Profit Growth 3Y (%)": profit_growth_display,
        "Profit Growth 5Y (%)": format_growth_pct(f.get("profit_growth_5y")),
        "Profit Growth 10Y/7Y (%)": format_growth_pct(f.get("profit_growth_10y") or f.get("profit_growth_7y")),
...
def main():
    ...
    # Cleanup orphaned temp files from interrupted prior runs.
    try:
        removed_tmp = cleanup_stale_temp_files(Path.cwd(), max_age_hours=12)
        if removed_tmp:
            logger.info(f"Removed {removed_tmp} stale temp files")
    except Exception:
        pass
    ...
    tmp_export = None
    try:
        with tempfile.NamedTemporaryFile(delete=False, prefix=TEMP_FILE_PREFIX, suffix=export_path.suffix or ".xlsx", dir=str(export_path.parent)) as tmp:
            tmp_export = tmp.name
        ...
        if tmp_export and Path(tmp_export).exists():
